 ...
 */
package etu2045.framework;

import etu2045.framework.AnnotationFonction;
import etu2045.framework.Mapping;
        ...
        try {
            ...
            for (Class<?> c : annoted_classes) {
                ...
                for (Method m : methods) {
                    if (m.isAnnotationPresent(etu2045.framework.Url.class)) {
                        Mapping mapping = new Mapping();
                        mapping.setClassName(c.getName());
                        mapping.setMethod(m.getName());
                        etu2045.framework.Url app = m.getAnnotation(etu2045.framework.Url.class);
                        String url = app.nom();
        ...
        try {
            ...
            if (this.MappingUrls.containsKey(url)) {
                ...
                try {
                    ...
                    for (Field f : field) {
                        if (f.getType() == etu2045.framework.UploadFile.class) {
